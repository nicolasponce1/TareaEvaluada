...
router.post('/new', (req, res) => {
  ...
  Order.createOrder(tableNumber, items, subtotal, total, (err, orderId) => {
    if (err) {
      return res.status(500).send('Error creating order'); // Envia una respuesta de error si hay un problema al crear el pedido
    }
    res.status(201).send(`Order created with ID: ${orderId}`); // Envia una respuesta de éxito con el ID del pedido creado
  });
});
